...
int main(int argc, char** argv) {
  ros::init(argc, argv, "transformation_visualization_node");//在程序初始化ros，"transformation_visualization_node"就是node(节点)的名字

  ros::NodeHandle node_handle;//nodeHandle是和ROS系统通讯的重要工具

  tf2_ros::Buffer tf_buffer;//定义tf_buffer为tf的缓冲器
  tf2_ros::TransformListener tf_listener(tf_buffer);//请求和接收坐标系变换信息
  ...
  // 服务客户端client_configure
  // 服务名称是"visualization_turtle_name + /set_pen"
  ...
  ros::Rate rate(10.0);//发布信息的速度为10Hz
